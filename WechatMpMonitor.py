...
def catch_sharing(msg):
	mpUserName=itchat.search_mps('中国移动10086')['UserName']  #查询“中国移动10086”公众号的UserName
	
	if msg['FromUserName'] == mpUserName:	#只监控“中国移动10086”公众号发布的文章
		url=msg['Url']
		urlCheckRegex=re.compile(r'(http://mp.weixin.qq.com/)(\w*)')
		urlCheckMo=urlCheckRegex.search(url)
		if urlCheckMo != None:	#公众号模板消息也是Sharing类型，但无法监控，因此需要过滤掉
